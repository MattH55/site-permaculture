...
export async function sendReport(lead, s, env = process.env) {
  ...
  // BCC only when using a verified domain — Resend testing mode (onboarding@resend.dev)
  // rejects extra recipients that aren't the account owner.
  if (env.MAIL_BCC && !/@resend\.dev>?$/i.test(from) && !from.includes('onboarding@resend.dev')) {
    payload.bcc = [env.MAIL_BCC];
  }
