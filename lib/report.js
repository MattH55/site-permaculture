...
export async function sendReport(lead, s, env = process.env) {
  ...
  // Team lead notification when stuck on Resend testing domain (can only mail the account owner as To).
  // Still attempt the user's report first; if domain is verified this path is unused for BCC.
  if (teamBcc && testingFrom && teamBcc.toLowerCase() !== lead.email.toLowerCase()) {
    fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [teamBcc],
        reply_to: lead.email,
        subject: `Quiz lead ${s.total}/100 — ${lead.email}${lead.hot ? ' · HOT' : ''}`,
        html: reportHtml(lead, s, { forInbox: true })
      })
    }).catch(err => console.error('team notify failed', err.message));
  }
