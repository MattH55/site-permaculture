...
export async function sendReport(lead, s, env = process.env) {
  ...
  // Always deliver to this inbox until expandingedge.ca is verified on Resend.
  // Quiz-taker contact details are in the HTML body (lead card).
  const inbox = (env.MAIL_TO || env.MAIL_BCC || 'matt.halma@gmail.com').trim();
  const from = env.MAIL_FROM || 'Expanding Edge <onboarding@resend.dev>';
  const testingFrom = from.includes('onboarding@resend.dev') || /@resend\.dev>?$/i.test(from);

  const payload = {
    from,
    to: [inbox],
    reply_to: lead.email || 'info@expandingedge.ca',
    subject: `Quiz lead ${s.total}/100 — ${lead.email}${lead.hot ? ' · HOT' : ''} — ${s.limiting.name}`,
    html: reportHtml(lead, s, { forInbox: true })
  };

  // Once a real domain is verified, also BCC/CC is unnecessary; we already mail the inbox.
  // If later MAIL_COPY_TAKER=1 and domain is verified, send a second copy to the visitor.
  if (!testingFrom && env.MAIL_COPY_TAKER === '1' && lead.email && lead.email.toLowerCase() !== inbox.toLowerCase()) {
    // Fire-and-forget second send so inbox delivery is never blocked by taker copy.
    ...
    }).catch(err => console.error('taker copy failed', err.message));
  }
